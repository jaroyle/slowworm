...
library(oSCR)
new1 <- oSCR.fit( model=list(D~1,p0~1,sig~1),scrFrame, ssDF=ssDF,start.vals=c(-3.6, log(20), -1) ,
plotit=FALSE, multicatch=TRUE , trimS=70)
new2 <- oSCR.fit( model=list(D~1,p0~session,sig~1),scrFrame, ssDF=ssDF,start.vals=c(-3.6, 0, 0, log(20), -1) ,
plotit=FALSE, multicatch=TRUE, trimS=70 )

new3 <- oSCR.fit( model=list(D~session,p0~1,sig~1),scrFrame, ssDF=ssDF,start.vals=c(-3.6, log(13), -3, 0, 0) ,
plotit=FALSE, multicatch=TRUE , trimS=70)

new4 <- oSCR.fit( model=list(D~session,p0~session,sig~1),scrFrame, ssDF=ssDF,start.vals=c(-2.9, -0.5, -1, log(13), -3, 0, 0) ,
plotit=FALSE, multicatch=TRUE, trimS=70 )

new5 <- oSCR.fit( model=list(D~session,p0~session + doy,sig~1),scrFrame, ssDF=ssDF,start.vals=c(-2.9, -0.5, -1, log(13)
,0, -4, 1.2, 1.2) ,
plotit=FALSE, multicatch=TRUE , trimS=70)

new6 <- oSCR.fit( model=list(D~session,p0~session + doy + I(doy^2),sig~1),scrFrame, ssDF=ssDF,
start.vals=c(-2.4, -1.2, -1.9, log(13)
,0,0, -4, 1.2, 1.2) ,
plotit=FALSE, multicatch=TRUE , trimS=70)

new7 <- oSCR.fit( model=list(D~session,p0~1 + doy  ,sig~1),scrFrame, ssDF=ssDF,
start.vals=c(-3.4, log(13)
,0, -4, 1.2, 1.2) ,
plotit=FALSE, multicatch=TRUE, trimS=70 )

new8 <- oSCR.fit( model=list(D~session,p0~1 + doy + I(doy^2)  ,sig~1),scrFrame, ssDF=ssDF,
start.vals=c(-3.4, log(13),0,0, -4, 0.8, 0.5), 
plotit=FALSE, multicatch=TRUE, trimS=70 )

fl<-fitList.oSCR(list(new1,new2,new3,new4,new5,new6,new7,new8),rename=TRUE)
modSel.oSCR(fl) -> ms

save(new1,new2,new3,new4,new5,new6,new7,new8,file="finalmodels.RData")
